...
fn test_cha_cha20_poly1305_does_support() {
    ...
    let km = tink::registry::get_key_manager(tink_testutil::CHA_CHA20_POLY1305_TYPE_URL)
        .expect("cannot obtain ChaCha20Poly1305 key manager");
    assert!(
        km.does_support(tink_testutil::CHA_CHA20_POLY1305_TYPE_URL),
...
fn validate_cha_cha20_poly1305_key(
    ...
    if key.version != tink_testutil::CHA_CHA20_POLY1305_KEY_VERSION {
        return Err(format!(
            ...
            tink_testutil::CHA_CHA20_POLY1305_KEY_VERSION
        )
        .into());
    }
    ...
    validate_cha_cha20_poly1305_primitive(tink::Primitive::Aead(Arc::new(p)), key)
}
