...
fn test_aes_gcm_get_primitive_basic() {
    tink_aead::init();
    let key_manager = tink::registry::get_key_manager(tink_tests::AES_GCM_TYPE_URL)
        .expect("cannot obtain AES-GCM key manager");
    assert_eq!(key_manager.type_url(), tink_tests::AES_GCM_TYPE_URL);
    assert_eq!(
        key_manager.key_material_type(),
        tink_proto::key_data::KeyMaterialType::Symmetric
    );
    for key_size in KEY_SIZES {
        let key = tink_tests::new_aes_gcm_key(tink_tests::AES_GCM_KEY_VERSION, *key_size);
...
fn test_aes_gcm_new_key_basic() {
    ...
    let key_manager = tink::registry::get_key_manager(tink_tests::AES_GCM_TYPE_URL)
        .expect("cannot obtain AES-GCM key manager");
    for key_size in KEY_SIZES {
        let format = tink_tests::new_aes_gcm_key_format(*key_size);
        let serialized_format = proto_encode(&format);
...
fn test_aes_gcm_new_key_data_basic() {
    ...
    for key_size in KEY_SIZES {
        ...
        let _key = tink_proto::AesGcmKey::decode(key_data.value.as_ref()).unwrap();
    }
}
...
fn test_aes_gcm_does_support() {
    ...
    let key_manager = tink::registry::get_key_manager(tink_tests::AES_GCM_TYPE_URL)
        .expect("cannot obtain AES-GCM key manager");
    assert!(
        key_manager.does_support(tink_tests::AES_GCM_TYPE_URL),
...
fn gen_invalid_aes_gcm_keys() -> Vec<Vec<u8>> {
    vec![
        // not a AES_GCMKey
        proto_encode(&tink_tests::new_aes_gcm_key_format(32)),
        // bad key size
        proto_encode(&tink_tests::new_aes_gcm_key(
...
fn gen_invalid_aes_gcm_key_formats() -> Vec<Vec<u8>> {
    vec![
        ...
        // invalid key size
        proto_encode(&tink_tests::new_aes_gcm_key_format(15)),
        proto_encode(&tink_tests::new_aes_gcm_key_format(23)),
        proto_encode(&tink_tests::new_aes_gcm_key_format(31)),
    ]
}

fn validate_aes_gcm_key(
    key: &tink_proto::AesGcmKey,
    format: &tink_proto::AesGcmKeyFormat,
) -> Result<(), TinkError> {
