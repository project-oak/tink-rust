...
/// It generates new [`AesGcmKey`](tink::proto::AesGcmKey) keys and produces new instances of
/// [`subtle::AesGcm`].
#[derive(Default)]
pub(crate) struct AesGcmKeyManager {}

impl tink::registry::KeyManager for AesGcmKeyManager {
    /// Create a [`subtle::AesGcm`] for the given serialized [`tink::proto::AesGcmKey`].
    fn primitive(&self, serialized_key: &[u8]) -> Result<tink::Primitive, TinkError> {
        if serialized_key.is_empty() {
            return Err("AesGcmKeyManager: invalid key".into());
        }
        let key = tink::proto::AesGcmKey::decode(serialized_key)
    ...
    fn new_key(&self, serialized_key_format: &[u8]) -> Result<Vec<u8>, TinkError> {
        ...
        let key_format = tink::proto::AesGcmKeyFormat::decode(serialized_key_format)
...
/// Validate the given [`tink::proto::AesGcmKey`].
fn validate_key(key: &tink::proto::AesGcmKey) -> Result<(), TinkError> {
...
/// Validate the given [`tink::proto::AesGcmKeyFormat`].
fn validate_key_format(format: &tink::proto::AesGcmKeyFormat) -> Result<(), TinkError> {
