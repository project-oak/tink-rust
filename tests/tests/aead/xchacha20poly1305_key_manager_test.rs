...
fn test_x_cha_cha20_poly1305_get_primitive() {
    tink_aead::init();
    let km = tink_core::registry::get_key_manager(tink_tests::X_CHA_CHA20_POLY1305_TYPE_URL)
    ...
    let key = tink_proto::XChaCha20Poly1305Key::decode(serialized_key.as_ref()).unwrap();
...
fn test_x_cha_cha20_poly1305_new_key_data() {
    ...
    let key = tink_proto::XChaCha20Poly1305Key::decode(kd.value.as_ref()).unwrap();
    validate_x_cha_cha20_poly1305_key(&key).unwrap();
}
...
fn gen_invalid_x_cha_cha20_poly1305_keys() -> Vec<tink_proto::XChaCha20Poly1305Key> {
    vec![
        // Bad key size.
...
fn validate_x_cha_cha20_poly1305_primitive(
    ...
    _key: &tink_proto::XChaCha20Poly1305Key,
) -> Result<(), TinkError> {
    let cipher = match p {
...
fn validate_x_cha_cha20_poly1305_key(
    ...
    validate_x_cha_cha20_poly1305_primitive(tink_core::Primitive::Aead(Box::new(p)), key)
}
