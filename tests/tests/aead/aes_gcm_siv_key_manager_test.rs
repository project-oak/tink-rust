...
fn test_aes_gcm_siv_get_primitive_basic() {
    tink_aead::init();
    let key_manager = tink_core::registry::get_key_manager(tink_tests::AES_GCM_SIV_TYPE_URL)
...
fn validate_aes_gcm_siv_key(
    ...
    validate_aes_gcm_siv_primitive(tink_core::Primitive::Aead(Box::new(p)), key)
}
