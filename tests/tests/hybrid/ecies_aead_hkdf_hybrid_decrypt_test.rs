...
fn modify_decrypt(curve: EllipticCurveType, k: tink_proto::KeyTemplate) {
    let pvt =
    ...
    for (i, g) in tink_tests::generate_mutations(&ct).iter().enumerate() {
        assert_ne!(g, &ct);
        assert!(
    ...
    for g in tink_tests::generate_mutations(&ct) {
        assert!(d.decrypt(&ct, &g).is_err(), "invalid context should fail");
    }

    for i in 0..salt.len() {
        for j in 0u8..8u8 {
            let mut m_salt = salt.clone();
            m_salt[i] ^= 1 << j;
            let d = tink_hybrid::subtle::EciesAeadHkdfHybridDecrypt::new(
                pvt.clone(),
                &m_salt,
